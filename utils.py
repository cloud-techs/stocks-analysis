...
def get_latest_beta():
    full_path_l = [os.path.join(os.path.dirname(__file__), 'csvs', i)
...
def get_last_csv():
    ...
    print("oldest date is ", last_date, last_csv)
    return last_csv
...
def get_latest_csv():
    ...
    for i in os.listdir(csv_path):
        ...
        csv_dates.append(i_date)
        #print(i_date, type(i_date))
    last_date = max(csv_dates)
    ld = last_date.strftime("%Y-%b-%d").split("-")
    last_csv = f"cm{ld[2]}{ld[1].upper()}{ld[0]}bhav.csv"
    print("Latest csv date is ", last_date, last_csv)
    return last_csv
...
def remove_last_csv(last_csv):
    ...
    print("Removed", last_csv)


def download_current_csv(delta):
    url, file_name, csv_name = form_file_name(delta)
    download_and_unzip(url, file_name, chunk_size=128)
...
def utils_data_refresh():
    ...
    for i in big_df.index.values:
        ...
        yield(i, high, low, hi, li)


def utils_csv_refresh(delta=0):
    last_csv = get_last_csv()
    print(last_csv)
    remove_last_csv(last_csv)
    download_current_csv(delta)
    get_latest_csv()
